...
def line_circle_intersection(line_type, circle_list, min_x=0, min_y=0, max_x=0, max_y=0, x=0, y=0):
    ...
    if line_type == 'horizontal':
        fac1 = circle_radius*circle_radius - math.pow(y-circle_y_center, 2)
        if fac1 < 0:
            return False
        else:
        ...
        fac1 = circle_radius * circle_radius - math.pow(x - circle_x_center, 2)
        if fac1 < 0:
            return False
        else:
...
def platform_collision(walker, platforms):
    ...
    for platform in platforms:
        if rectangle_rectangle_intersection(walker.rect_list(), platform.rect_list()):
        ...
        if abs(walker.vel_y) > platform.height:
            ...
            if past_player_bottom < platform_top and current_player_bottom > platform_bottom:
                ...
                for j in range(num_steps - 1):
                    ...
                    if rectangle_rectangle_intersection(past_player_rect, platform.rect_list()):
                        # Landing on top of platform
